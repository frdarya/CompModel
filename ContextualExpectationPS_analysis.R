...
t1 <- glmer(targ ~ targ_cond + (1|subjRandID), family = binomial, data = targ_1st)
summary(t1) # n.s.
Anova(t1)
eff1<-effect('targ_cond',t1,se=TRUE)

# f1 first
f1_1st <- subset(ds, f1_pos == 1)
f1_1 <- glmer(f1 ~ f1_cond +(1|subjRandID), data = f1_1st, family = binomial)
summary(f1_1) 
Anova(f1_1)
plot(allEffects(f1_1))
eff2<-effect('f1_cond',f1_1,se=TRUE)

#f2 first
f2_1st <- subset(ds, f2_pos == 1)
f2_1 <- glmer(f2 ~ f2_cond +(1|subjRandID), data = f2_1st, family = binomial)
summary(f2_1) 
Anova(f2_1)
plot(allEffects(f2_1))
eff3<-effect('f2_cond',f2_1)

# f3 first
f3_1st <- subset(ds, f3_pos == 1)
f3_1 <- glmer(f3 ~ f3_cond +(1|subjRandID), data = f3_1st, family = binomial)
summary(f3_1) #n.s.
Anova(f3_1)
plot(allEffects(f3_1))
eff4<-effect('f3_cond',f3_1)
